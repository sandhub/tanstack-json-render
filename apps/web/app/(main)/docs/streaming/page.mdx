...
For custom or framework-agnostic streaming implementations, use the SpecStream compiler from `@tanstack-json-render/core` directly:
...
`@tanstack-json-render/react` provides the `useUIStream` hook, which wraps the low-level compiler in a React-friendly API with state management, error handling, and abort support.
...
See the [@tanstack-json-render/react API reference](/docs/api/react) for full `useUIStream` documentation.

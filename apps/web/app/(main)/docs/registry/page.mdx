...
- **`@tanstack-json-render/react`** — Components (React elements) and action handlers
- **`@tanstack-json-render/react-native`** — Components (React Native elements) and action handlers
- **`@tanstack-json-render/remotion`** — Clip components, transitions, and effects
...
Actions are declared in your [catalog](/docs/catalog). The `@tanstack-json-render/react` schema supports an `actions` key where you define what operations AI can trigger:
...
For read-only state access (e.g. displaying a value from state), use `$state` expressions in props — they are resolved before the component receives them. For custom logic, use `useStateStore` and `getByPath` from `@tanstack-json-render/core`.
...
`@tanstack-json-render/react-native` uses the same `defineRegistry` API. The only difference is that components return React Native elements instead of HTML:

```tsx
import { defineRegistry } from '@tanstack-json-render/react-native';
...
See the [@tanstack-json-render/react-native API reference](/docs/api/react-native) for the full API.

## @tanstack-json-render/remotion

`@tanstack-json-render/remotion` takes a different approach. Instead of `defineRegistry`, it uses a plain component registry with built-in standard components for video production:

```tsx
import { Renderer, standardComponents } from '@tanstack-json-render/remotion';
...
See the [@tanstack-json-render/remotion API reference](/docs/api/remotion) for the full API.

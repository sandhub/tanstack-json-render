...
Use json-render with [TanStack AI](https://tanstack.com/ai) for seamless streaming. json-render supports two modes: **Generate** (standalone UI) and **Chat** (UI embedded in conversation). See [Generation Modes](/docs/generation-modes) for a detailed comparison.
...
export async function POST(req: Request) {
  ...
  const stream = chat({
    ...
    systemPrompts: [systemPrompt + contextPrompt],
    temperature: 0.7,
  });
...
import { useUIStream, Renderer } from "@tanstack-json-render/react";
...
export async function POST(req: Request) {
  ...
  const stream = chat({
    adapter: anthropicText("claude-haiku-4-5-20251001"),
    messages,
    systemPrompts: [catalog.prompt({ mode: "chat" })],
    temperature: 0.7,
  });

  const uiStream = createUIMessageStream({
    execute: async ({ writer }) => {
      writer.merge(pipeJsonRender(stream));
    },
  });
...
Use `useChat` from TanStack AI React and `useJsonRenderMessage` from json-render to extract the spec from each message:

```tsx
"use client";

import { useChat, fetchServerSentEvents } from "@tanstack/ai-react";
import { useJsonRenderMessage, Renderer } from "@tanstack-json-render/react";

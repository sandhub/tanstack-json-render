...
Build your own schema and renderer with `@tanstack-json-render/core`.

## Overview

`@tanstack-json-render/core` is schema-agnostic. While `@tanstack-json-render/react` provides a ready-to-use schema and renderer, you can create your own to match any JSON structure - whether it's a domain-specific format, an existing protocol, or something entirely custom.

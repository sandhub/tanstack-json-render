...
### Package Rename: `@json-render/*` → `@tanstack-json-render/*`

All public packages have been renamed from `@json-render/*` to `@tanstack-json-render/*` to align the npm scope with the project identity. Update your imports and dependencies:
...
All packages follow the same pattern: `@json-render/core` → `@tanstack-json-render/core`, `@json-render/react` → `@tanstack-json-render/react`, etc. Internal packages (`@internal/*`) are unchanged.

### New: `@tanstack-json-render/image`

Server-side image renderer powered by [Satori](https://github.com/vercel/satori). Turns the same `{ root, elements }` spec format into SVG or PNG output for OG images, social cards, and banners.
...
```typescript
import { renderToSvg, renderToPng } from "@tanstack-json-render/image/server";
...
const spec: Spec = {
  ...
  elements: {
    frame: {
      type: "Frame",
      props: { width: 1200, height: 630, background: "#1a1a2e" },
      children: ["title"],
    },
    title: {
      type: "Heading",
      props: { text: "Hello World", color: "#ffffff", fontSize: 48 },
      children: [],
    },
  },
};
...
const png = await renderToPng(spec); // requires @resvg/resvg-js
```

9 standard components: Frame, Box, Row, Column, Heading, Text, Image, Divider, Spacer. Sub-path exports: `/render`, `/catalog`, `/server`.

See the [Image API reference](/docs/api/image) for details.

### Migrated: AI SDK → TanStack AI

All server and client AI integration has been migrated from the Vercel AI SDK (`ai`) to [TanStack AI](https://tanstack.com/ai) (`@tanstack/ai`). This affects streaming, chat hooks, and server-side utilities.
...
New core utilities: `streamToTextResponse()` for simple text streaming responses, and updated `StreamChunk` type for TanStack AI's chunk format.
...
### New: `@tanstack-json-render/vue`

Vue 3 renderer for json-render with full feature parity with `@tanstack-json-render/react`. Data binding, visibility conditions, actions, validation, repeat scopes, streaming, and external store support.
...
### New: `@tanstack-json-render/xstate`
...
```bash
npm install @tanstack-json-render/xstate @xstate/store
```
...
`@tanstack-json-render/react`, `@tanstack-json-render/react-pdf`, and `@tanstack-json-render/react-native` v0.9.0 failed to install because `@internal/react-state` (a private workspace package) was published as a dependency. The internal package is now bundled into each renderer at build time, so it no longer needs to be resolved from npm.
...
New adapter packages: `@tanstack-json-render/redux`, `@tanstack-json-render/zustand`, `@tanstack-json-render/jotai`.
...
`@tanstack-json-render/react` barrel-imports React contexts that call `createContext`, which crashes in Next.js App Router API routes (RSC runtime strips `createContext`). All docs, examples, and skills now import `schema` from `@tanstack-json-render/react/schema` instead of `@tanstack-json-render/react`.
...
Fixed an issue where chaining multiple actions on the same event (e.g. `setState` followed by a custom action) did not execute all actions. Affected `@tanstack-json-render/react`, `@tanstack-json-render/react-native`, and `@tanstack-json-render/react-pdf`.
...
PDF renderer for json-render, powered by [`@react-pdf/renderer`](https://react-pdf.org/). Define catalogs and registries the same way as `@tanstack-json-render/react`, but output PDF documents instead of web UI.
...
Supports custom catalogs with `defineRegistry`, server-safe imports via `@tanstack-json-render/react-pdf/server`, and full context support (state, visibility, actions, validation, repeat scopes).
...
### New: `@tanstack-json-render/shadcn`
...
const raw = chat({ adapter: anthropicText("claude-haiku-4-5-20251001"), messages, systemPrompts: [system] });
...
### New: @tanstack-json-render/react-native
...
### New: @tanstack-json-render/remotion
...
- Schema is now imported from renderer packages (`@tanstack-json-render/react`) not core
...
- Added `@tanstack-json-render/codegen` package (spec traversal and JSX serialization)
- Configurable AI model via environment variables
- Documentation improvements and bug fixes

*Note: Only @tanstack-json-render/core was published to npm for this release.*

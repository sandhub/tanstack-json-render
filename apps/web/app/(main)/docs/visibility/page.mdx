...
In `@tanstack-json-render/react`, wrap your app with `VisibilityProvider` to enable conditional rendering. The `Renderer` handles visibility automatically — elements with unmet conditions are not rendered.

```tsx
import { VisibilityProvider, StateProvider } from '@tanstack-json-render/react';
...
```tsx
import { useIsVisible } from '@tanstack-json-render/react';
...
See the [@tanstack-json-render/react API reference](/docs/api/react) for full details.

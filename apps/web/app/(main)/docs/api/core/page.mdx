...
json-render uses a flexible schema system that defines both the AI output format (spec) and what catalogs must provide. Each renderer package provides its own schema (e.g., @tanstack-json-render/react exports `schema`).

### schema

The schema for flat UI element trees. This is exported from @tanstack-json-render/react.
...
```typescript
import { VisibilityConditionSchema } from '@tanstack-json-render/core';
...
```typescript
import { compileSpecStream } from '@tanstack-json-render/core';
...
```typescript
import { applySpecPatch } from '@tanstack-json-render/core';
...
```typescript
import { nestedToFlat } from '@tanstack-json-render/core';
...
```typescript
import { createMixedStreamParser } from '@tanstack-json-render/core';
...
Async generator that intercepts a TanStack AI chat stream and classifies text content as either prose or json-render JSONL patches. Lines that parse as JSONL patches are emitted as `spec` chunks; everything else passes through as `content` chunks. Used in Chat mode API routes.
...
```typescript
import { getByPath, setByPath } from '@tanstack-json-render/core';
...
```typescript
import { resolveDynamicValue } from '@tanstack-json-render/core';

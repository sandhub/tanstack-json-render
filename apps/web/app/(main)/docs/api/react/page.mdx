...
Catalog-agnostic base type for building reusable component libraries (e.g. `@tanstack-json-render/shadcn`) that are not tied to a specific catalog:
...
```typescript
import { buildSpecFromParts, getTextFromParts } from '@tanstack-json-render/react';

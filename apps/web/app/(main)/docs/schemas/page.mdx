...
json-render can work with any JSON schema. `@tanstack-json-render/core` provides the primitives to define catalogs and renderers for any format:

- **@tanstack-json-render/react** — The built-in flat element tree schema
...
`@tanstack-json-render/react` uses a flat element tree schema with a root key and elements map:
...
`@tanstack-json-render/core` is schema-agnostic. You can define any JSON structure:

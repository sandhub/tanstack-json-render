...
`defineCatalog` is from `@tanstack-json-render/core`. The `schema` import comes from your platform package (`@tanstack-json-render/react` or `@tanstack-json-render/react-native`) and defines the element structure the catalog targets. The catalog definition itself is framework-agnostic.

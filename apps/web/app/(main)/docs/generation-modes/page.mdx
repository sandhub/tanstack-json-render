...
On the client, use `useUIStream` from `@tanstack-json-render/react` or the lower-level `createSpecStreamCompiler` from `@tanstack-json-render/core` to compile the JSONL stream into a spec:
...
import { createUIMessageStream, createUIMessageStreamResponse } from "@tanstack/ai";
...
On the client, use `useJsonRenderMessage` from `@tanstack-json-render/react` to extract the spec from a chat message's parts:
...
- See the [TanStack AI integration](/docs/ai-sdk) for setup with TanStack AI

...
State can also be provided programmatically at runtime. In `@tanstack-json-render/react`, this is done via `StateProvider` and hooks like `useStateStore`. See the [React API reference](/docs/api/react) for details.

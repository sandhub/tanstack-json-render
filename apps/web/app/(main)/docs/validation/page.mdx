...
import { schema } from '@tanstack-json-render/react/schema'; // or '@tanstack-json-render/react-native/schema'
import { z } from 'zod';
...
In `@tanstack-json-render/react`, use `ValidationProvider` to supply implementations for your custom validators:

```tsx
import { ValidationProvider } from '@tanstack-json-render/react';
...
```tsx
import { useFieldValidation, useBoundProp } from '@tanstack-json-render/react';
...
See the [@tanstack-json-render/react API reference](/docs/api/react) for full `ValidationProvider` and `useFieldValidation` documentation.
